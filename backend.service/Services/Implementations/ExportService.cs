...
namespace backend.service.Services.Implementations
{
    public class ExportService : IExportService
    {
        ...
        private (byte[] Bytes, string FileName) GenerateSingleAccountExcelExport(List<VwTransactionsList> transactions, string? accountName, long? accountNumber, string? bankName, string fy, int count, bool separateSheets, bool mergeDescriptions = false)
        {
            using var workbook = new XLWorkbook();
        ...
        private static void BuildAccountSheet(XLWorkbook wb, string? accountName, long? accountNumber, string? bankName, List<VwTransactionsList> transactions, string fy)
        {
        ...
        private static void WriteSheetHeaderInline(IXLWorksheet ws, ref int currentRow, string? title, string fy, long? accountNumber = null, string? bankName = null)
        {
            if (accountNumber.HasValue)
                title = $"{title} ({accountNumber.Value})";
        ...
        private static void WriteTransactionRows(IXLWorksheet ws, ref int currentRow, List<VwTransactionsList> transactions, bool isAccountLayout)
        {
            ...
            foreach (var t in transactions)
            {
                ...
                ws.Cell(currentRow, 3).Value = isAccountLayout
                    ? t.DescriptionName
                    : (t.AccountNumber.HasValue ? $"{t.AccountName} ({t.AccountNumber.Value})" : t.AccountName);
        ...
        private static string GetUniqueSheetName(XLWorkbook wb, string baseName, long? accountNumber = null)
        {
        ...
        private async Task<AccountExportContext> ResolveAccountContextAsync(ExportRequestModel request)
        {
            ...
            if (equalsFilter != null && !string.IsNullOrEmpty(equalsFilter.Value?.ToString()))
            {
                ...
                var acc = await _unitOfWork.GetRepository<Account>().SingleOrDefaultAsync(a => a.AccountSID == sid);
                if (acc != null)
                {
            ...
            if (inFilter?.Value is JsonElement je && je.ValueKind == JsonValueKind.Array)
            {
                ...
                if (sids.Count == 1)
                {
                    var acc = await _unitOfWork.GetRepository<Account>().SingleOrDefaultAsync(a => a.AccountSID == sids[0]);
                    if (acc != null)
                    {
                        var entry = new AccountEntry(acc.AccountSID, acc.AccountName, acc.AccountNumber, acc.BankName);
                        return new AccountExportContext(AccountExportMode.Single, new List<AccountEntry> { entry });
        ...
        private static (string, int) SanitizeFileName(string? name, int counter, long? accountNumber = null)
        {
        ...
        private record AccountEntry(string? AccountSID, string? AccountName, long? AccountNumber, string? BankName);
